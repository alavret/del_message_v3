...
- **Переменные окружения**: Все обязательные параметры (`OAUTH_TOKEN_ARG`, `ORGANIZATION_ID_ARG`, `DELEGATE_ALIAS`, `DELEGATE_DOMAIN`, `DELEGATE_PASSWORD`, `RUN_MODE`) должны быть заданы. При их отсутствии скрипт завершится с кодом `1`.
...
- подключаться к Общим ящикам (недоступно для токено сервисного приложения);
...
Поэтому добавлен режим hybrid, при котором необходимо иметь как настроенную учётную запись делегата (`DELEGATE_ALIAS`,`DELEGATE_DOMAIN`, `DELEGATE_PASSWORD`), так и корректно настроенно сервисное приложение (`APPLICATION_CLIENT_ID_ARG`, `APPLICATION_CLIENT_SECRET_ARG`).
...
- пропиши параметры сервисного приложения и делегата в `.env` и в RUN_MODE укажите `hybrid`.
